...
class GenderService {
  ...
  Future<Map<String, dynamic>> predictGender(File imageFile) async {
    try {
      ...
      if (crop == null) {
        developer.log('[GenderService] No face detected in image', name: 'GenderService');
      ...
      developer.log('[GenderService] Input buffer shape: $inputShape', name: 'GenderService');
      ...
      developer.log('[GenderService] Output buffer shape: $outputShape', name: 'GenderService');
      ...
      developer.log('[GenderService] Parsed prediction: $parsed', name: 'GenderService');
  ...
  Future<math.Rectangle<int>?> _detectLargestFaceCrop(File imageFile) async {
    ...
    developer.log('[GenderService] Detecting face (ML Kit)...', name: 'GenderService');
    final inputImage = InputImage.fromFilePath(imageFile.path);
    final faces = await _faceDetector!.processImage(inputImage);
    developer.log('[GenderService] Faces detected: ${faces.length}', name: 'GenderService');
  ...
  Map<String, dynamic> _parseGenderOutput(dynamic output, List<int> shape) {
    ...
    if (shape.length == 2 && shape[1] == 2 && output is List && output.isNotEmpty) {
      final a = (output[0][0] as num).toDouble();
      final b = (output[0][1] as num).toDouble();
      developer.log('[GenderService] Raw predictions - idx0: $a, idx1: $b', name: 'GenderService');
      // Swap mapping because model outputs were reversed (idx0=female, idx1=male).
      return b > a ? {'gender': 'male', 'confidence': b} : {'gender': 'female', 'confidence': a};
    }

    if (shape.length == 2 && shape[1] == 1 && output is List && output.isNotEmpty) {
      final p = (output[0][0] as num).toDouble();
      developer.log('[GenderService] Raw prediction - p: $p', name: 'GenderService');
      // Assumption: p is probability of male (0..1)
      final isMale = p >= 0.5;
      return {'gender': isMale ? 'male' : 'female', 'confidence': isMale ? p : (1.0 - p)};
    }
